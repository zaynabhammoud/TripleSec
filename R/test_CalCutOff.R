...
inputData <- inputDataAll[-1,] #create inputData, table containing all but one of the given patients
pHigh <- 0.8 #create pHigh
pLow <- 0.2 #create pLow

fit <- calCutOff(inputData, pHigh, pLow) #run calCutOff
print(fit) # print fit (with all its links)
print(fit$cutOff) # print resulting cutoff table
